...
% Only firing rates are returned.
%
...
function runSimFR(parname, Nn, seed)

% Time the network simulation
tic

run(strcat('parameter_files/',parname,'.m'));
folderName = parname
N = Nn
...
% Copy file with file name parname+'.m' to folder results/parname
source_file = fullfile('parameter_files', [parname, '.m']);
...
disp(sprintf('\nSimulation Complete.\n'));
save(sprintf('sim_results/%s/frates_%d_seed_%d.mat', parname, N, seed),'frates');
...
%subplot(4,1,2)
%X=dX:dX:Np;
%plot(X,Iapp(Ne+1:Ne+Np,:))
%ylabel('PV rate (Hz)')
%subplot(4,1,3)
%X=dX:dX:Ns;
%plot(X,Iapp(Ne+Np+1:Ne+Np+Ns,:))
%ylabel('SST rate (Hz)')
%subplot(4,1,4)
%X=dX:dX:Nv;
%plot(X,Iapp(Ne+Np+Ns+1:Ne+Np+Ns+Nv,:))
%ylabel('VIP rate (Hz)')
%xlabel('cell location')
...
%print(sprintf('sim_results/%s/raster_%s.svg', parname, N), '-dsvg', '-r300');
%print(sprintf('sim_results/%s/frates_%d.png', parname, N), '-dpng', '-r300');
